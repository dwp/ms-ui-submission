...
import { JourneyContext, ValidationError } from '@dwp/govuk-casa';

const { DEFAULT_CONTEXT_ID } = JourneyContext;
chai.use(sinonChai);
...
describe('JourneyContext', () => {
  ...
  describe('set/getData*()', () => {
    it('should return undefined if I request data for a non-existent page', () => {
      ...
      return expect(data1.unknownpage).to.be.undefined;
    });
    ...
    it('should retrieve data for a specific page that I pass into it', () => {
      ...
      expect(JSON.stringify(data2.data.pageA)).to.equal(JSON.stringify(dataset2.pageA));
    });
    ...
    it('should retrieve data correctly for a page when that data is swapped out', () => {
      ...
      data3.pageA = dataset.pageA;
      expect(JSON.stringify(data3.pageA)).to.equal(JSON.stringify(dataset.pageA));
      data3.pageA = swapset;
      expect(JSON.stringify(data3.pageA)).to.equal(JSON.stringify(swapset));
    });

    it('should throw a TypeError when getData() is passed an incorrectly typed argument', () => {
    ...
    it('should throw a TypeError when setData() is passed an incorrectly typed argument', () => {
      const c = new JourneyContext();
  ...
  describe('validateContextId', () => {
    ...
    it('should throw if the provided ID is not a valid UUID', () => {
      expect(() => {
        JourneyContext.validateContextId('abcdefgh-e89b-12d3-a456-a426614174000.');
  ...
  describe('putContext()', () => {
    ...
    it('should initialise session if not already initialised', () => {
      ...
      expect(initSpy).to.have.been.calledOnceWithExactly(session);
      initSpy.restore();
    });
  });
