...
export default function staticRouter({
  ...
  const setHeaders = (req, res, next) => {
    ...
    } else if (pathname.includes('timeout')) {
      // Just needed for our in-memory CSS assets
  ...
  // The CASA CSS / timeout.js source contains the placeholder `~~~CASA_MOUNT_URL~~~` which
  ...
  const stylesCss = readFileSync(resolve(dirname, '../../dist/assets/css/ms-ui-submission.css'), { encoding: 'utf8' });
  const timeoutJs = readFileSync(resolve(dirname, '../../dist/assets/js/timeout.js'), { encoding: 'utf8' });
  ...
  router.get('/casa/assets/css/ms-ui-submission.css', setHeaders, (req, res) => res.send(stylesCss.replace(/~~~CASA_MOUNT_URL~~~/g, validateUrlPath(`${req.baseUrl}/`))));
  router.get('/casa/assets/js/timeout.js', setHeaders, (req, res) => res.send(timeoutJs.replace(/~~~CASA_MOUNT_URL~~~/g, validateUrlPath(`${req.baseUrl}/`))));
