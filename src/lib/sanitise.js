 ...
 */
export function sanitiseWaypoint(waypoint = '') {
    if (typeof waypoint !== 'string') {
        return '';
    }

    return waypoint.toLowerCase().replace(/[^a-z0-9/-]/g, '').replace(/\/+/g, '/').replace(/^\/+/, '').replace(/\/+$/, '');
}
 ...
 * @returns {string} Path
 */
export function sanitiseAbsolutePath(path) {
 ...
 */
export function sanitiseRelativeUrl(url) {
...
export function sanitiseContextId(id) {
    if (typeof id !== 'string') {
        return '';
    }
