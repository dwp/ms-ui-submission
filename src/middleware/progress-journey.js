...
export default ({
  ...
  (req, res, next) => {
    ...
    if (req.casa.editMode && req.casa.editOrigin) {
      ...
      // Remove URL params after changing an answer and been redirected to CYA
      const editOrigin = new URLSearchParams(url.search).get('editorigin');
      const urlString = editOrigin === '/check-your-answers' ? '' : url.search.toString();
      const redirectUrl = waypointUrl({ waypoint: url.pathname }) + urlString;
