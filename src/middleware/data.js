...
const editOrigin = (req) => {
  if (has(req && req.query, 'editorigin')) {
    return waypointUrl({ waypoint: req.query.editorigin });
  }
  if (has(req && req.body, 'editorigin')) {
  ...
  return [
    (req, res, next) => {
      ...
      req.casa = {
        ...
        // Edit mode
        editMode: (has(req && req.query, 'edit') && has(req && req.query, 'editorigin')) || (has(req && req.body, 'edit') && has(req && req.body, 'editorigin')),
