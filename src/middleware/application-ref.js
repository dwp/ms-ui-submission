...
export default (router, redis, startPage) => {
  router.use(startPage, (req, res, next) => { // eslint-disable-line consistent-return
    ...
    if (typeof redis !== 'undefined') {
      redis.incr('esa:ref').then((int) => {
        const sessionRef = req.session.id
          ...
          .substring(0, 4)
          .toUpperCase();
        const randomStr = Math.random().toString(36).replace(/[^a-z]+/g, '').substr(0, 4)
          .toUpperCase();
